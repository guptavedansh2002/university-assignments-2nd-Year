...
{
    int temp_srno, moreiter;
    i = 0;
    while (true)
    {
        printf("\nEnter the Serial Number corresponding to the component need to be entered: ");
        scanf("%d", &temp_srno);
        if (temp_srno>size || temp_srno<=0)
        {
            printf("Enter the serial number between 1 and %d as per limit!", size);
        }
        else
        {
            a[temp_srno].sr_no = temp_srno;
            printf("\nEnter the component %d name: ", i+1);
            scanf("%s", &a[temp_srno].comp);
            printf("\nEnter the component %d symbol: ", i+1);
            scanf("%s", &a[temp_srno].symb);
            printf("\nEnter the component %d value: ", i+1);
            scanf("%d", &a[temp_srno].val);
            printf("\nEnter the component %d cost: ", i+1);
            scanf("%f", &a[temp_srno].cost);
        }
        i++;
        printf("\nIf your %d entries are complete, Enter 1, or if not, Enter 0.", size);
        scanf("%d", &moreiter);
        if (moreiter == 1)
        {
            break;
...
int main()
{
    ...
    if (size>5)
    {
        printf("Size of table should be between 1 and 5. Please try entering again.");
        return 0;
    }
