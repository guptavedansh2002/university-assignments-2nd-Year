//Program to perform OR operation
...
int main()
{
    ...
    //Using OR operation to the binary codes of the two numbers entered by the user.
    //Setting the final count to iterate through the two arrays cause of different count of elements in the two integer arrays. For example, if inpin1 had total 9 valuable elements which are needed to compute OR and inpin2 had 11. This causes a difference in number of iterations required in total to perform OR, and might ignore the 3 more binary in inpin2. Thus final count is set as 11 and the 3 extra binary in inpin1 is already set to 0 which will contribute to calculating the AND.
    count = (c1 > c2) ? c1 : c2;
